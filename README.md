...
**지도 기반 모임 탐색**과 **AI 맞춤 모임 추천**을 통한 **취미 공유** 커뮤니티 플랫폼입니다.
...
| 이름     | 문제점                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | 해결 방법                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **민기** | 1. **낙관적 업데이트로 인한 알림 읽음 처리 오류**: 알림 클릭 시 읽음 처리가 정상적으로 반영되지 않는 문제가 발생. 낙관적 업데이트로 UI를 먼저 갱신한 뒤 서버 요청이 실패해도 롤백 처리가 없어 UI와 서버 상태가 불일치하는 현상이 원인<br/>2. **프로필 수정 후 정보가 업데이트되지 않는 문제**: 수정 페이지에서 프로필 정보를 변경한 뒤 저장해도 데이터가 반영되지 않는 문제 발생. Zustand 스토어에서 관리하는 값(토큰, ID, 이미지 경로, 지역 등)을 `FormData`에 포함시키지 않아 Server Action에 데이터가 전달되지 않는 것이 원인<br/>3. **카카오맵 컴포넌트 공유 시 디자인 깨짐 현상**: 메인 페이지와 지도 페이지에서 동일한 카카오맵 컴포넌트를 사용하는 과정에서, 각 페이지의 지도 표시 크기가 달라 디자인이 깨지는 현상 발생 | 1. **롤백 로직 추가**: 서버 요청 실패 시 이전 상태로 되돌리는 롤백 로직을 추가하여 UI와 서버 간 데이터 정합성을 확보<br/>2. **hidden input 활용**: `input type="hidden"`을 활용하여 Zustand 스토어 및 클라이언트 상태 값을 폼에 포함시키고, `formAction` 호출 시 `FormData`를 통해 서버에 정상적으로 전달되도록 구조를 수정<br/>3. **props 기반 클래스명 분리**: 컴포넌트에 props를 전달하여 현재 페이지를 구분하고, 이를 기반으로 CSS 클래스명을 분리하여 페이지별로 독립적인 스타일을 적용 |
| **현욱** | **북마크 컴포넌트 대량 API 요청 문제**: 북마크 컴포넌트를 처음 구현할 때는 각 모임의 meetingId를 기준으로 개별적으로 API를 호출하여 북마크 여부를 확인하고, 추가 및 취소 요청을 처리하는 방식으로 설계했음. 단일 상세 페이지에서는 큰 문제가 없었지만, 모임 리스트 페이지처럼 여러 개의 북마크 컴포넌트가 동시에 렌더링되는 화면에서는 모임 개수만큼 API 요청이 발생하는 문제가 생김. 예를 들어 모임이 100개라면 최대 100번의 요청이 발생할 수 있어 서버에 너무 많은 요청을 보내는 문제를 겪음 | **전역 상태 관리로 전환**: API 호출 방식을 컴포넌트 단위에서 전역 상태 관리 방식으로 변경함. 사용자가 로그인할 때 북마크한 모든 모임을 한 번의 API 요청으로 일괄 조회하고, 해당 데이터를 bookmarkStore에 저장하도록 구조를 개선함. 이후 각 북마크 컴포넌트는 개별적으로 API를 호출하는 대신, 전역 상태에 저장된 데이터를 참조하여 북마크 여부를 판단하도록 수정함. 또한 북마크 토글 시에는 서버에 한 번만 요청을 보내고, 성공 시 store의 상태를 즉시 업데이트하여 UI를 동기화하도록 설계함 |
| **현주** | **북마크 다중 선택 버그**: 모임 리스트에서 북마크 버튼을 클릭하면 해당 모임만 선택되어야 하는데, 여러 개의 북마크가 동시에 선택되는 버그가 발생. 원인을 추적한 결과 `git pull`을 받지 않아 팀원이 수정한 최신 북마크 로직이 반영되지 않은 상태에서 작업을 진행한 것이 원인이었음 | **최신 코드 동기화**: `git pull`과 `git merge`를 통해 `develop` 브랜치의 최신 코드를 로컬에 동기화하여 해결. 이후 작업 시작 전 항상 `git pull`로 최신 코드를 받는 습관을 정착시킴 |
...
│   │   ├── ChatRoomItem.tsx       # 채팅방 아이템
│   │   ├── ChatBadge.tsx          # 채팅 알림 뱃지
